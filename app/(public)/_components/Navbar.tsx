    ...
    <>
      <div className="fixed left-[50%] top-0 z-50 hidden h-[70px] w-full -translate-x-[50%] items-center justify-between px-4 text-white backdrop-blur-3xl md:flex md:px-6 lg:px-10">
        ...
        <div className="flex items-center gap-3">
          <Link
            href="/login"
            className={`${isLoggedIn ? "hidden" : "block"} text-white hover:bg-white/8 hover:text-white`}
          >
            Log in
          </Link>
          <Link
          ...
          >
            {isLoggedIn ? "Dashboard" : "Start your journey"}
          </Link>
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-40 md:hidden">
          ...
          <div className="absolute right-4 top-20 w-[calc(100vw-2rem)] max-w-[320px] rounded-2xl border border-white/10 bg-[#1a1a1a]/98 p-4 shadow-xl backdrop-blur-3xl">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <Link
                href="/login"
                className={`${isLoggedIn ? "hidden" : "block"} w-full justify-center text-white hover:bg-white/8 hover:text-white`}
                onClick={closeMobileMenu}
              >
                Log in
              </Link>
              <Link
                href={`${isLoggedIn ? "/dashboard" : "/signup"}`}
                className="w-full justify-center rounded-full italic bg-white px-5 text-black hover:bg-white/90"
                onClick={closeMobileMenu}
              >
                {`${isLoggedIn ? "Dashboard" : "Start your journey"}`}
              </Link>
